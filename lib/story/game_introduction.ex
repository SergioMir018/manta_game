defmodule Story.GameIntroduction do
  @moduledoc """
  Module containing major narrative game introduction components
  """
  ...
  def introduction do
    ...
    travel_to_town(gender)
  end

  def travel_to_town(gender) do
    TextFormatter.print_with_wrap(
      "El carruaje en el que viajas avanza lentamente por un angosto camino muy irregular y mal cuidado, rodeado de densos árboles. Justo a la llegada de un hueco en la espesura, a lo lejos, las primeras casas de Manta se perfilan como sombras bajo un cielo gris y triste, que se ocultan detrás de una poco usual neblina a la que no estás acostumbrad#{gender}. La sensación de quietud en el aire resulta ser inquietante, y el silencio de la ruta solo es roto por el sonido de las ruedas del carruaje sobre el barro y el choque regular contra las piedras del camino.\n"
    )

    IO.puts("Pulsa ENTER para continuar...")
    IO.gets("")
    TerminalUtils.clear_screen()
    first_entry_town(gender)
  end

  def first_entry_town(gender) do
    TextFormatter.print_with_wrap(
    ...
    sheriff_first_meet(gender)
  end

  def sheriff_first_meet(gender) do
    TextFormatter.print_with_wrap(
    ...
    TextFormatter.print_with_wrap(
      "Sin decir una palabra baja tu equipaje del carruaje y le hace una seña al conductor para que se ponga en marcha nuevamente. Se para frente a tí con un rostro serio y te mira de arriba ha abajo, como si te estuviera midiendo de alguna forma. Puedes sentir que está siendo cauteloso antes de siquiera dirigirte la palabra. Se estira empidando el pecho y mirandote fijamente a los ojos.\n"
    )

    TextFormatter.print_with_wrap("Policía desconocido -> 'Bienvenid#{gender} detective'")
    TextFormatter.print_with_wrap("Dice sin mucha calidez.")

    IO.gets("")

    TextFormatter.print_with_wrap(
      "Policía desconocido -> 'L#{gender} estabamos esperando. Yo soy el Alguacil de este pueblo. Me puede llamar Rand. Pasemos dentro de la comisaría, necesitamos hablar'\n"
    )
  end
end
